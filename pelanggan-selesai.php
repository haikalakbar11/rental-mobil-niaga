...
// Validasi 
$sql = $connection->query("SELECT a.id_mobil, a.tgl_ambil, a.lama FROM transaksi a WHERE a.id_mobil=$_POST[id_mobil] AND a.status='0'"); if ($sql->num_rows) {     $d = $sql->fetch_assoc(); 
    $sql = "SELECT 
      (SELECT (( 
        DATEDIFF(ADDDATE('$tgl_ambil', INTERVAL $_POST[lama] DAY), 
ADDDATE('$d[tgl_ambil]', INTERVAL $d[lama] DAY)) 
      )) FROM transaksi WHERE id_mobil=$d[id_mobil] LIMIT 1) AS a,       (SELECT (( 
        DATEDIFF(ADDDATE('$d[tgl_ambil]', INTERVAL $d[lama] DAY), 
ADDDATE('$tgl_ambil', INTERVAL $_POST[lama] DAY)) 
      )) FROM transaksi WHERE id_mobil=$d[id_mobil] LIMIT 1) AS b"; 
    $s = $connection->query($sql); 
    $a = $s->fetch_assoc(); 
    if ($a["a"] == 0 AND $a["b"] == 0) { 
        echo alert("Maaf, mobil yang anda sewa sudah di pesan!");         exit; 
    } 
} 
$query = $connection->query("SELECT * FROM mobil WHERE id_mobil=$_POST[id_mobil]"); $data  = $query->fetch_assoc(); 
 
$hargasupir  = 0; 
$id          = $_SESSION["pelanggan"]["id"];  
$jatuhtempo  = date('Y-m-d H:i:s', strtotime('+3 hours')); $totalbayar  = $hargasupir + ($data["harga"] * $_POST["lama"]); if ($_POST["status"]) $hargasupir = (30000 * $_POST["lama"]); 
 
$connection->query("INSERT INTO transaksi VALUES (NULL, $id, $_POST[id_mobil], 
'$now', '$tgl_ambil', NULL, $_POST[lama], $totalbayar, '0', '$_POST[jaminan]', NULL, 
'$jatuhtempo', '0', '0')"); 
$idtransaksi = $connection->insert_id; 
 
if ($_POST["status"]) {     $hargasupir = 30000; 
...
<div class="card card-info"> 
    <div class="card-header"><h3 class="text-center">Transaksi Berhasil</h3></div> 
    <div class="card-body"> 
        ...
        <p> 
            ...
            <strong>(97236271 a/n SEWA MOBIL)</strong> untuk menyelesaikan pembayaran. dan untuk uang muka minimal setengah dari harga sewa. 
        </p>         <p> 
    ...
    <div class="card-footer"> 
        <a href="?page=profil" class="btn btn-primary btn-sm">Lihat Profil</a>     </div> 
</div> 
<br> 
