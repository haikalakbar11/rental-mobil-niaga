...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
 	if ($update) { 
 	 	$sql = "UPDATE pelanggan SET no_ktp='$_POST[no_ktp]', nama='$_POST[nama]', email='$_POST[email]', alamat='$_POST[alamat]', telp='$_POST[no_telp]', username='$_POST[username]'";  	 	if ($_POST["password"] != "") { 
 	 	 	$sql .= ", password='".md5($_POST["password"])."'"; 
 	 	} 
 	 	$sql .= " WHERE id_pelanggan='$_GET[key]'"; 
 	} else { 
 	 	$sql = "INSERT INTO pelanggan VALUES (NULL, '$_POST[no_ktp]', 
'$_POST[nama]', '$_POST[email]', '$_POST[alamat]', '$_POST[no_telp]', 
...
if (isset($_GET['action']) AND $_GET['action'] == 'delete') {   $connection->query("DELETE FROM pelanggan WHERE id_pelanggan='$_GET[key]'"); 
	 	echo alert("Berhasil!", "?page=pelanggan"); 
} 
?> 
...
"EDIT PELANGGAN" : "TAMBAH PELANGGAN" ?></h3></div> 
	 	        <div class="card-body"> 
                     ...
                     <input type="text" name="nama" class="form-control" <?= (!$update) ?: 'value="'.$row["nama"].'"' ?> placeholder="Masukkan Nama Lengkap"> 
 	                    ...
 	                    <input type="text" name="no_ktp" class="form-control" <?= (!$update) ?: 'value="'.$row["no_ktp"].'"' ?> placeholder="Masukkan No KTP">  	                </div> 
	 	                <div class="form-group"> 
	 	                    <label for="no_telp">Telp</label> 
 	                    <input type="text" name="no_telp" class="form-control" <?= (!$update) ?: 'value="'.$row["no_telp"].'"' ?> placeholder="Masukkan No Telp">  	                </div> 
	 	                <div class="form-group"> 
...
(!$update) ?: 'value="'.$row["alamat"].'"' ?> placeholder="Masukkan Alamat">  	                </div> 
	 	                <div class="form-group"> 
                     ...
                     <?php if ($query = $connection->query("SELECT * FROM pelanggan")): ?> 
 	 	 	 	 	...
 	 	 	 	 	<td><?=$row['alamat']?></td> 
 	                            ...
 	                            <td class="d-print-none" style="text-align: center;"> 
 	                                <div class="btn-group"> 
 	                                    <a 
href="?page=pelanggan&action=update&key=<?=$row['id_pelanggan']?>" class="btn btn-warning btn-sm">Edit</a> 
 	                                    <a 
href="?page=pelanggan&action=delete&key=<?=$row['id_pelanggan']?>" class="btn btn-danger btn-sm">Hapus</a> 
 	                                </div> 
 	                            </td> 
 	                        </tr> 
 	                        <?php endwhile ?> 
