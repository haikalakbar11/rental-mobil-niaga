...
email='$_POST[email]', alamat='$_POST[alamat]', telp='$_POST[telp]', 
username='$_POST[username]'";  	 	if ($_POST["password"] != "") { 
 	 	 	$sql .= ", password='".md5($_POST["password"])."'"; 
 	 	} 
 	 	$sql .= " WHERE id_admin='$_GET[key]'"; 
 	} else { 
 	 	$sql = "INSERT INTO admin VALUES (NULL, '$_POST[nama]', 
'$_POST[email]', '$_POST[alamat]', '$_POST[telp]', '$_POST[username]', 
'".md5($_POST["password"])."')"; 
 	} 
  if ($connection->query($sql)) {     echo alert("Berhasil!", "?page=admin"); 
  } else { 
 	 	echo alert("Gagal!", "?page=admin"); 
  } 
} 
if (isset($_GET['action']) AND $_GET['action'] == 'delete') { 
  $connection->query("DELETE FROM admin WHERE id_admin='$_GET[key]'");  	echo alert("Berhasil!", "?page=admin"); 
} 
?> 
...
"EDIT ADMIN" : "TAMBAH ADMIN" ?></h3></div> 
 	        <div class="card-body"> 
...
"info" ?> btn-block">Simpan</button> 
	 	                <?php if ($update): ?> 
	 	 	 	 	 	 	 	 	 	 	<a 
href="?page=admin" class="btn btn-info btn-block">Batal</a> 
 	 	 	 	 	 	 	 	 	<?php endif; ?> 
	 	            </form> 
	 	        </div> 
	 	    </div><br> 
                     ...
                     <?php if ($query = $connection->query("SELECT * FROM admin")): ?> 
 	 	 	 	 	...
 	 	 	 	 	<td><?=$row['alamat']?></td> 
 	                            <td class="d-print-none" style="text-align: center;"> 
 	                                <div class="btn-group"> 
 	                                    <a 
href="?page=admin&action=update&key=<?=$row['id_admin']?>" class="btn btn-warning btn-sm">Edit</a> 
 	                                    <a 
href="?page=admin&action=delete&key=<?=$row['id_admin']?>" class="btn btn-danger btn-sm">Hapus</a> 
 	                                </div> 
 	                            </td> 
 	                        </tr> 
 	                        <?php endwhile ?> 
